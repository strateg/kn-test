...
public class ServiceCallingRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        try {
            ...
            Log.warn("Latest exchange rate for pair {}/{} is : {}", baseCurrency, targetCurrency, rate.toString());

            ExchangeRate lowestRate = demoExchangeService.lowestRateOfCurrentMonth(baseCurrency, targetCurrency);
            Log.warn("Lowest exchange rate for current month for pair {}/{} is : {}", baseCurrency, targetCurrency, lowestRate.toString());

            ExchangeRate highestRate = demoExchangeService.highestRateOfCurrentMonth(baseCurrency, targetCurrency);
            Log.warn("Highest exchange rate for current month for pair {}/{} is : {}", baseCurrency, targetCurrency, highestRate.toString());
